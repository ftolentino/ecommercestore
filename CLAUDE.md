...
- **`ply-classes.json` is not exhaustive — the CSS is.** The whole `.alert` family (`alert`, `alert-red`, `alert-blue`, `alert-outline`, `alert-icon`, `alert-dismiss`, …) ships in `dist/css/ply.css` but is **missing from the JSON**, even though PLY.md demos it and calls the JSON the source of truth. Treat the JSON as the fast path, and `grep dist/css/ply.css` as the authority. The reverse also holds: never use a class you have confirmed in neither.
